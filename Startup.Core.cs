...
namespace HostPanelPro.Web.Services;
...
public static class StartupCore
{
    ...
    public const int MaxBytesPerRead = 32 * MB;
    public const int MaxDepth = 32 * MB;
    public const int MaxArrayLength = 32 * MB;
    public const int MaxStringContentLength = 32 * MB;
    public const int MaxNameTableCharCount = 32 * MB;
	...
	public static void Init(string[] args)
	{
		...
		builder.WebHost.UseKestrel(options =>
		{
			...
			if (HttpPort.HasValue && !HttpFile.HasValue) options.ListenAnyIP(HttpPort.Value);
		});
	...
	public static void ConfigureServices(IServiceCollection services)
	{
		...
		var srvcAssemblies = new[] { "HostPanelPro.Server" }
			.Select(assembly => assembly.Replace('.', ' '))
		...
		services
			...
			.AddServiceModelWebServices(o =>
			{
				...
				o.Description = $"This is the REST API of HostPanelPro. Note that all {openServices}services use Basic Http Authentication. If you use .NET, you might want to access the API over WCF/SOAP, in this case refer to the {clientAssembly}assembly.";
				o.TermsOfService = new("http://hostpanelpro.com/terms");
				o.ContactName = "Contact";
				o.ContactEmail = "support@hostpanelpro.com";
				o.ContactUrl = new("http://hostpanelpro.com/contact");
				o.ExternalDocumentUrl = new("http://hostpanelpro.com/apidoc.pdf");
	...
	public static IServiceBuilder AddServiceEndpoint(this IServiceBuilder builder, Type service, Type contract, Binding binding, Uri address)
	{
		...
		builder.AddServiceEndpoint(service, contract, binding, address, address);
		return builder;
	}
	...
	public static void ConfigureWCF(IApplicationBuilder app)
	{
		...
		{
			var webServices = new[] { new ServiceType(typeof(HostPanelPro.Server.Services.Test)) };

			var isLocal = true;

			foreach (var ws in webServices)
			{
				Debug.WriteLine($"Configuring WebService {ws.Service.Name}");
			...
			builder.ConfigureAllServiceHostBase(host =>
			{
				...
				foreach (var endpoint in host.Description.Endpoints)
				{
					if (endpoint.Binding is WebHttpBinding &&
						!endpoint.EndpointBehaviors.OfType<SoapHeaderMessageInspector>().Any())
					{
